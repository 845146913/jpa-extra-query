...
public class JpaQueryUtils {
    ...
    private static final String replacePointSeparator = "0_";
    ...
    private static void wrapForObject(Object object, StringBuilder dynamicSql, String alias, String whereOrAnd, String match) {
    ...
    private static void addCondition(StringBuilder dynamicSql, String alias, String whereOrAnd, SearchFilter filter) {
        ...
        if (fieldName.contains(pointSeparator)) {
            ...
            aliasName = split[0];
            field = split[1];
            fieldName = fieldName.replace(pointSeparator, replacePointSeparator);
        }
    ...
    private static void setSearchFilterParameter(Query query, SearchFilter criter) {
        ...
        switch (op) {
            case AND:
            case OR: {
                throw new IllegalArgumentException(" SearchFilter does not support the AND,OR Operator, please use LogicSearchFilter! ");
            }
            case NOTIN:
            case IN: {
                Object value = filter.getValue();
    ...
    public static String getJpqlTableAlias(String jpql, String simpleName) {
        String alias = "";
        int simpleNameIdx = jpql.lastIndexOf(simpleName);
        if (simpleNameIdx <= 0) {
    ...
    public static List<Field> getDeclaredFieldsForClass(Class<?> object) {
        List<Field> fields = new ArrayList<Field>();
    ...
    public static String getCountSqlString(String jpql, String alias) {
        int fromIdx = jpql.toLowerCase().indexOf(" from ");
        if (fromIdx <= 0) {
            throw new IllegalArgumentException("jpql is non-standard sql");
        }
        // distinct
        boolean distinct = jpql.toLowerCase().contains(" distinct ");
        if (distinct) {
            alias = alias.substring(0, alias.lastIndexOf(pointSeparator));
            return "select count(distinct " + alias + ") " + jpql.substring(fromIdx);
        }
    ...
    public static String getSortCondition(Sort sort, String alias) {
        if (Objects.nonNull(sort) && sort.isSorted()) {
            ...
            for (String order : split) {
                ...
                if (key.contains(pointSeparator)) {
